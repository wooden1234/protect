...
class MutilScal(nn.Module):
    def __init__(self, dim=48, fc_ratio=4, dilation=[3, 5, 7], pool_ratio=16):
...
class Block(nn.Module):
    def __init__(self, dim=48, num_heads=16,  mlp_ratio=4, pool_ratio=16, drop=0., dilation=[3, 5, 7],
...
class Decoder(nn.Module):
    def __init__(self,
                 # encode_channels=[32,48,80,160,256],
                 encode_channels=[64, 128, 256, 512, 1024],
                 decode_channels=128,
                 dilation = [[1, 3, 5], [3, 5, 7], [5, 7, 9], [7, 9, 11],[9, 11, 13]],
                 fc_ratio=4,
                 dropout=0.1,
                 num_classes=6):
        ...
        self.b5 = Block(dim=decode_channels, num_heads=16, mlp_ratio=4, pool_ratio=16, dilation=dilation[0])

        self.p4 = Fusion(decode_channels)
        self.b4 = Block(dim=decode_channels, num_heads=16, mlp_ratio=4, pool_ratio=16, dilation=dilation[1])

        self.p3 = Fusion(decode_channels)
        self.b3 = Block(dim=decode_channels, num_heads=16, mlp_ratio=4, pool_ratio=16, dilation=dilation[2])

        self.p2 = Fusion(decode_channels)
        self.b2 = Block(dim=decode_channels, num_heads=16, mlp_ratio=4, pool_ratio=16, dilation=dilation[3])

        self.Conv4 = ConvBN(encode_channels[-4], encode_channels[-5], 1)

        self.p1 = Fusion(encode_channels[-5])
        self.seg_head = MAF(encode_channels[-5], fc_ratio=fc_ratio, dilation=dilation[4], dropout=dropout, num_classes=num_classes)

        self.init_weight()

    def forward(self, x1,x2,x3,x4,x5, h, w):
...
class DepthwiseSeparableConv(nn.Module):
    def __init__(self, ch_in, ch_out, norm_layer=None):
        ...
        if norm_layer is None:
            norm_layer = lambda c: nn.GroupNorm(4, c)
...
class UNet(nn.Module):
    def __init__(self, n_channels=4, base_filters=32):
        super(UNet, self).__init__()
        self.n_channels = n_channels
        filters = [base_filters * (2 ** i) for i in range(5)]  # [32, 64, 128, 256, 512]

        self.Maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.Conv1 = DepthwiseSeparableConv(self.n_channels, filters[0])
...
class UnetCMTF(nn.Module):
    def __init__(self,
                 ...
                 dropout=0.1,
                 num_classes=2,
                 backband = UNet(n_channels=4,base_filters=64)
                 ):
        super().__init__()

        self.backband = backband
        self.decoder = Decoder(encode_channels, decode_channels, dropout=dropout, num_classes=num_classes)
        self.n_classes = num_classes

    def forward(self, x):
        h, w = x.size()[-2:]
        x1, x2, x3, x4, x5= self.backband(x)
        # print(x5.shape)
        # print(x4.shape)
        # print(x3.shape)
        # print(x2.shape)
        # print(x1.shape)
        x = self.decoder(x1,x2,x3,x4,x5, h, w)
        return x
...
'''if __name__ == '__main__':
    ...
    x = torch.randn(1, 4, 256, 256).to(device)  # 将输入张量移动到 GPU 1
    net = CMTFNet().to(device)  # 将模型移动到 GPU 1
    out = net(x)
