...
class MAF(nn.Module):
    def __init__(self, dim, fc_ratio, dilation=[3, 5, 7], dropout=0., num_classes=6):
        ...
        self.head = nn.Sequential(SeparableConvBNReLU(dim, dim, kernel_size=3),
                                  ...
                                  Conv(192, num_classes, kernel_size=1))
...
class Wnet(nn.Module):
    def __init__(self, n_channels=3, n_classes=7, scale_factor=1,os=16):
        ...
        filters = np.array([48, 80, 160, 256])
        ...
        # 四个不同带洞卷积的设置，获取不同感受野
        self.aspp1 = ASPP_module(256, 48, dilation=dilations[0])
        self.aspp2 = ASPP_module(256, 48, dilation=dilations[1])
        self.aspp3 = ASPP_module(256, 48, dilation=dilations[2])
        self.aspp4 = ASPP_module(256, 48, dilation=dilations[3])
        self.relu = nn.ReLU()
        self.global_avg_pool = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)),
                                             nn.Conv2d(256, 48, 1, stride=1, bias=False),
                                             nn.GroupNorm(1,48),
                                             nn.ReLU())

        self.CBAM = CBAM(in_chans=48)
        self.MHSA =Mutilscal_MHSA(dim=160,num_heads=16)
        self.EFFN = E_FFN(in_features=160,out_features=48)
        self.CB =ConvBN(240,filters[-4],1)
        self.CBR =ConvBNReLU(filters[-3],filters[-4],1)
        self.MAF =MAF(dim=192,fc_ratio=4,dropout=0.1,num_classes=n_classes)
