...
class APFormerHeadMulti(BaseDecodeHead):
    ...
    def __init__(self, feature_strides, pool_scales=(1, 2, 3, 6), **kwargs):
        ...
        self.attn_c4 = Block(dim1=c4_in_channels, dim2=tot_channels + c3_in_channels * num_Multi,
                             num_heads=num_heads[0], mlp_ratio=4,
                             drop_path=0.1, pool_ratio=8)
        self.attn_c3 = Block(dim1=c3_in_channels, dim2=tot_channels + c3_in_channels * num_Multi,
                             num_heads=num_heads[1], mlp_ratio=4,
                             drop_path=0.1, pool_ratio=4)
        self.attn_c2 = Block(dim1=c2_in_channels, dim2=tot_channels + c3_in_channels * num_Multi,
                             num_heads=num_heads[2], mlp_ratio=4,
                             drop_path=0.1, pool_ratio=2)
        self.attn_c1 = Block(dim1=c1_in_channels, dim2=tot_channels + c3_in_channels * num_Multi,
                             num_heads=num_heads[3], mlp_ratio=4,
                             drop_path=0.1, pool_ratio=1)

        self.cat_key1 = CatKeyMulti(pool_ratio=pool_ratio,
                                    dim=[c4_in_channels, c3_in_channels, c2_in_channels, c1_in_channels],
                                    num_feat=num_Multi)
        self.cat_key2 = CatKeyMulti(pool_ratio=pool_ratio,
                                    dim=[c4_in_channels, c3_in_channels, c2_in_channels, c1_in_channels],
                                    num_feat=num_Multi)
        self.cat_key3 = CatKeyMulti(pool_ratio=pool_ratio,
                                    dim=[c4_in_channels, c3_in_channels, c2_in_channels, c1_in_channels],
                                    num_feat=num_Multi)
        self.cat_key4 = CatKeyMulti(pool_ratio=pool_ratio,
                                    dim=[c4_in_channels, c3_in_channels, c2_in_channels, c1_in_channels],
                                    num_feat=num_Multi)
