...
class ReparamConv(nn.Module):
    ...
    def __init__(self, in_channels, expand_channels, out_channels, large_kernel_size, kernel_size, stride=1, groups=1,
                 deploy=False):
...
class MobileBlock(nn.Module):
    ...
    def __init__(self, in_channels, expand_channels, out_channels, large_kernel_size, kernel_size, ):
        ...
        self.depthwise_conv_l = nn.Sequential(
            nn.Conv2d(expand_channels, expand_channels, kernel_size=5, stride=1, padding=2, dilation=1,
...
class DeformConv(nn.Module):
    def __init__(self, input_dim, output_dim, stride=1, padding=[1, 1], dilation=[1, 1]):
        ...
        self.double_conv_l = nn.Sequential(
            ...
            DeformConv_V2(output_dim, output_dim, kernel_size=3, stride=stride, padding=padding[0],
                          dilation=dilation[0]),
        ...
        self.double_conv_r = nn.Sequential(
            ...
            nn.BatchNorm2d(output_dim),
            nn.LeakyReLU(inplace=True),
            DeformConv_V2(output_dim, output_dim, kernel_size=3, stride=stride, padding=padding[1],
                          dilation=dilation[1]),
...
class Carafe_Up(nn.Module):
    def __init__(self, input_decoder, output_dim, compressed_channels=64, scale_factor=2):
        ...
        self.carafe_up = nn.Sequential(nn.BatchNorm2d(input_decoder),
                                       ...
                                       CARAFEPack(input_decoder, scale_factor=scale_factor,
