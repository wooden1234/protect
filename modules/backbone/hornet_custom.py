...
from modules.block.conv_custom import AdaptiveDilatedConv, AdaptiveDilatedDWConv

if 'DWCONV_IMPL' in os.environ:
    try:
        sys.path.append(os.environ['DWCONV_IMPL'])
        from modules.block.DepthWiseConv2dImplicitGEMM import DepthWiseConv2dImplicitGEMM


        def get_dwadconv(dim, kernel, bias):
        ...
        def get_dwadconv(dim, kernel, bias):
            ...
            return AdaptiveDilatedDWConv(dim, dim, kernel_size=kernel, padding=(kernel - 1) // 2, bias=bias, groups=dim)
            # return ModulatedDeformConv2dPack(dim, dim, kernel_size=kernel, padding=(kernel-1)//2 ,bias=bias, groups=dim)
...
class GlobalLocalFilter(nn.Module):
    def __init__(self, dim, h=14, w=8, kernel_size=3, use_adaptive_dilation=True, normal_conv_dim=0):
        super().__init__()
        if use_adaptive_dilation:
            self.dw = AdaptiveDilatedDWConv(dim // 2, dim // 2, kernel_size=kernel_size, padding=1, bias=False,
                                            groups=dim // 2, normal_conv_dim=normal_conv_dim)
        else:
...
class gnconv(nn.Module):
    def __init__(self, dim, order=5, gflayer=None, h=14, w=8, s=1.0, use_adaptive_dilation=False):
        super().__init__()
        self.order = order
        self.dims = [dim // 2 ** i for i in range(order)]  # C, C/2, C/4
        self.dims.reverse()  # small to large
        self.proj_in = nn.Conv2d(dim, 2 * dim, 1)

        if gflayer is None:
            self.dwconv = get_dwconv(sum(self.dims), 7, True)
        else:
            self.dwconv = gflayer(sum(self.dims), h=h, w=w, use_adaptive_dilation=use_adaptive_dilation,
                                  normal_conv_dim=sum(self.dims[1:]) // 2)
...
class gnadconv(nn.Module):
    def __init__(self, dim, order=5, gflayer=GlobalLocalFilter, h=14, w=8, s=1.0):
        ...
        if gflayer is None:
            self.dwconv = get_dwadconv(sum(self.dims), 7, True)
        else:
...
class Block(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, gnconv=gnadconv):
        super().__init__()
...
class HoradNet(nn.Module):
    ...
    def __init__(self, in_chans=4, num_classes=2,
        ...
        self.freeze_train_list = freeze_train_list
        ...
        if len(freeze_train_list) > 0:
            self._freeze_all()

    def _freeze_all(self):
        print("*" * 20, "freeze all but:", )
        for i in range(0, 4):
            m = self.downsample_layers[i]
            # m.eval()
            # for param in m.parameters():
            #     if 'FrequencySelection' not in param:
            #         param.requires_grad = False
            for name, param in m.named_parameters():
                # print(name)
                if any(i in name for i in self.freeze_train_list):
                    m.train()
