...
def na2d_av(attn: torch.Tensor,
    ...
    B, G, H, W, C = value.shape
    _, _, K, H2, W2 = attn.shape

    assert K == kernel_size * kernel_size, f"K={K} 与 kernel_size={kernel_size} 不匹配"
    assert (H, W) == (H2, W2), "value 和 attn 的空间尺寸不一致"

    BG = B * G
    v = value.permute(0, 1, 4, 2, 3).contiguous().view(BG, C, H, W)  # 转成 (B*G, C, H, W)
    a = attn.view(BG, K, H, W)

    pad = ((kernel_size - 1) // 2) * dilation
    patches = F.unfold(v, kernel_size=kernel_size, dilation=dilation, padding=pad)
    patches = patches.view(BG, C, K, H, W)
    out = (a.unsqueeze(1) * patches).sum(dim=2)
    out = out.view(B, G, C, H, W).permute(0, 1, 3, 4, 2)  # 输出 (B, G, H, W, C)
    return out
...
def get_conv2d(in_channels,
    ...
    if attempt_use_lk_impl and need_large_impl:
        ...
        try:
            from modules.block.DepthWiseConv2dImplicitGEMM import DepthWiseConv2dImplicitGEMM_1
            print('---------------- found iGEMM implementation ')
        except:
            DepthWiseConv2dImplicitGEMM_1 = None
            print('---------------- found no iGEMM. use original conv. follow https://github.com/AILab-CVC/UniRepLKNet to install it.')
        if DepthWiseConv2dImplicitGEMM_1 is not None and need_large_impl and in_channels == out_channels \
                and out_channels == groups and stride == 1 and dilation == 1:
            print(f'===== iGEMM Efficient Conv Impl, channels {in_channels}, kernel size {kernel_size} =====')
            return DepthWiseConv2dImplicitGEMM_1(in_channels, kernel_size, bias=bias)
...
class DynamicConvBlock(nn.Module):
    ...
    def _forward_inner(self, x, h_x, h_r):
        ...
        attn1 = torch.softmax(attn1, dim=-1).permute(0, 1, 4, 2, 3)  # [B, G, 25, H, W]
        attn2 = torch.softmax(attn2, dim=-1).permute(0, 1, 4, 2, 3)  # [B, G, 169, H, W]
        value = rearrange(x, 'b (m g c) h w -> m b g h w c', m=2, g=self.num_heads)
        # print("weight.shape:", weight.shape)
        # print("self.smk_size:", self.smk_size, "self.kernel_size:", self.kernel_size)
        # print("expected split sizes:", self.smk_size ** 2, self.kernel_size ** 2)
        # print(f"attn1 shape: {attn1.shape}")  # 预期 (B, G, H, W, K)
        # print(f"value[0] shape: {value[0].shape}")  # 预期 (B, G, H2, W2, C)
