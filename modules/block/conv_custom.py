...
class AdaptiveDilatedDWConv(ModulatedDeformConv2d):
    ...
    def ad_forward(self, x):
        ...
        if hasattr(self, 'OMNI_ATT1') and hasattr(self, 'OMNI_ATT2'):
            ...
            # print("adaptive_weight.shape:", adaptive_weight.shape)
            # print("adaptive_weight.numel():", adaptive_weight.numel())
...
#     def __init__(self, in_channels, out_channels, freq_band=4, kernel_size=1, dilation=1):
#         super().__init__()
#         self.freq_band = freq_band
#         self.kernel = nn.Conv2d(
#             in_channels * freq_band * 2,  # sin+cos
#             out_channels,
...
#             freq = 2 ** i
#             freq_encodings.append(torch.sin(x * freq))
#             freq_encodings.append(torch.cos(x * freq))
#         x_encoded = torch.cat(freq_encodings, dim=1)  # (B, 2*freq_band*C, H, W)
#         return self.kernel(x_encoded)
#
# class FLC_Pooling(nn.Module):
#     def __init__(self, freq_thres=0.25):
#         """
#         freq_thres: 控制低通范围的比例（0-0.5），越小保留频率越低
#         """
...
#
#         # 对每个通道做 2D 傅里叶变换
#         x_fft = torch.fft.fft2(x, norm='ortho')               # 傅里叶变换
#         x_fft = torch.fft.fftshift(x_fft, dim=(-2, -1))       # 中心化频谱
#
#         # 构造低通掩码
#         freq_mask = self._make_low_pass_mask(H, W, device=x.device)
#
#         # 应用低通滤波器
...
#         x_low = torch.fft.ifft2(x_fft_filtered, norm='ortho').real  # 取实部
#
#         return x_low
#
#     def _make_low_pass_mask(self, H, W, device):
#         """创建一个低通掩码圆形区域"""
#         center_x, center_y = H // 2, W // 2
#         Y, X = torch.meshgrid(torch.arange(H), torch.arange(W), indexing='ij')
#         dist = ((X - center_x) ** 2 + (Y - center_y) ** 2).sqrt()
#         radius = self.freq_thres * min(H, W)
#         mask = (dist <= radius).float()
#         mask = mask.to(device)
#         return mask.unsqueeze(0).unsqueeze(0)  # shape (1, 1, H, W)
#
# class StaticLP(nn.Module):
#     def __init__(self, in_channels, kernel_size=3, stride=1, padding=1, alpha=8, use_gaussian=True):
#         """
#         in_channels: 输入通道数
#         kernel_size: 卷积核大小
#         alpha: 高斯控制因子（越大越模糊）
#         use_gaussian: 是否使用高斯核，否则使用均值核
#         """
#         super(StaticLP, self).__init__()
#         self.in_channels = in_channels
#         self.kernel_size = kernel_size
#         self.stride = stride
...
#
#         # 创建卷积核（固定）
#         kernel = self._create_kernel(kernel_size, alpha, use_gaussian)
#         kernel = kernel.repeat(in_channels, 1, 1, 1)  # shape: (C, 1, K, K)
#
#         # 注册为 buffer，表示它不是参数
#         self.register_buffer('weight', kernel)
#
#     def _create_kernel(self, k, alpha, use_gaussian):
#         if use_gaussian:
#             # 生成高斯核
#             center = k // 2
#             x = torch.arange(k).float()
#             gauss = torch.exp(-((x - center) ** 2) / (2 * (alpha ** 2)))
#             kernel = gauss[:, None] @ gauss[None, :]
#             kernel = kernel / kernel.sum()
#         else:
#             # 使用均值核
#             kernel = torch.ones((k, k)) / (k * k)
...
class MultiFreqBandConv(nn.Module):
    """将输入分解到不同频率带分别处理的多频带卷积

    Args:
        in_channels (int): 输入通道数
        out_channels (int): 输出通道数
        freq_band (int): 频带划分数量（默认4）
        kernel_size (int/tuple): 卷积核大小
        dilation (int/tuple): 空洞率
    """
    ...
    def __init__(self, in_channels, out_channels, freq_band=4, kernel_size=1, dilation=1):
        ...
        self.freq_attention = nn.Sequential(
            ...
            nn.Conv2d(in_channels, freq_band, kernel_size=1),
            nn.Softmax(dim=1)
        )
    ...
    def forward(self, x):
        ...
        for i in range(self.freq_band):
            ...
            h_start, h_end = int(h * i / self.freq_band), int(h * (i + 1) / self.freq_band)
            w_start, w_end = int(w * i / self.freq_band), int(w * (i + 1) / self.freq_band)
            mask[..., h_start:h_end, w_start:w_end] = 1

            # 提取当前频带并逆变换
            band = torch.fft.ifft2(torch.fft.ifftshift(x_fft * mask), norm='ortho').real
            outputs.append(self.conv_bands[i](band))

        # 频带注意力加权融合
        attn = self.freq_attention(x)
        return sum([out * attn[:, i:i + 1] for i, out in enumerate(outputs)])
...
class FLC_Pooling(nn.Module):
    ...
    def forward(self, x):
        ...
        radius = int(min(h, w) * self.freq_thres)

        # 计算频率坐标距离
        y = torch.arange(h, device=x.device).view(1, 1, h, 1) - center_h
        x_coord = torch.arange(w, device=x.device).view(1, 1, 1, w) - center_w
        dist = torch.sqrt(y ** 2 + x_coord ** 2)

        # 应用掩码并逆变换
        mask = (dist >= radius).float()
        return torch.fft.ifft2(torch.fft.ifftshift(x_fft * mask), norm='ortho').real
...
class StaticLP(nn.Module):
    ...
    Args:
        ...
        alpha (float): 权重衰减系数（控制滤波强度）
    """

    def __init__(self, channels, kernel_size=3, alpha=8):
        super().__init__()
        self.kernel_size = kernel_size
        ...
        for i in range(kernel_size):
            for j in range(kernel_size):
                ...
                weight[:, :, i, j] = alpha / (dist + alpha)

        # 归一化保证能量守恒
        self.weight = nn.Parameter(weight / weight.sum(dim=(2, 3), keepdim=True),
                                   requires_grad=False)

    def forward(self, x):
        # 分组卷积实现逐通道滤波
        return F.conv2d(x, self.weight, padding=self.kernel_size // 2, groups=x.size(1))