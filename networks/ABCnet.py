...
from networks.ABCresnet import Resnet18
...
class ABCNet(nn.Module):
    def __init__(self, band, n_classes):
        ...
        self.name = 'ABCNet'
        self.cp = ContextPath()
        self.sp = SpatialPath(band)
        self.n_classes = n_classes
        self.fam = FeatureAggregationModule(256, 256)
        self.conv_out = Output(256, 256, n_classes, up_factor=8)
        self.conv_fuse = nn.Conv2d(in_channels=3*n_classes, out_channels=n_classes, kernel_size=1)
        if self.training:
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            '''print('feat_out16.shape',feat_out16.shape)
            print('feat_out32.shape', feat_out32.shape)
            print('feat_out.shape', feat_out.shape)'''
            combined_features = torch.cat([feat_out,feat_out16,feat_out32],dim=1)
            #print('combined_features.shape', combined_features.shape)
            #self.conv_fuse = nn.Conv2d(in_channels=6, out_channels=2, kernel_size=1)
            fuse_out = self.conv_fuse(combined_features)
...
'''if __name__ == "__main__":
    ...
    net.cuda()
    net.train()
    in_ten = torch.randn(4, 4, 512, 512).cuda()
    out = net(in_ten)
    print(out[0].shape)
    print(out[1].shape)
    print(out[2].shape)
...
# 输出网络输出
    ...
    print("output_tensor的形状：",output_tensor.shape)