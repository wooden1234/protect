...
class ResNet(nn.Module):
    def __init__(self, block, layers, strides, norm_layer=None):
        ...
        self.norm_layer = norm_layer
        pretrained = model_zoo.load_url(model_urls['resnet18'])
        ...
        self.conv1.weight.data[:, :3] = pretrained['conv1.weight']
        self.conv1.weight.data[:, 3] = self.conv1.weight.data.mean(dim=1)
    ...
    '''def init_weight(self,state_dict):
        ...
        pretrained = model_zoo.load_url(model_urls['resnet18'])
        self.conv1.weight.data[:, :3] = pretrained['conv1.weight']
        self.conv1.weight.data[:, 3] = self.conv1.weight.data[:, :3].mean(dim=1)
    ...
    def init_weight(self, state_dict):
        ...
        for k, v in state_dict.items():
            if 'fc' in k: continue
            if k == 'conv1.weight':
                self_state_dict[k][:, :3] = v
                self_state_dict[k][:, 3] = self_state_dict[k][:, :3].mean(dim=1)
            else:
                self_state_dict[k] = v
        self.load_state_dict(self_state_dict, strict=True)
