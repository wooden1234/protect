...
class MANet(nn.Module):
    def __init__(self, num_channels=3, num_classes=5):
        ...
        #self.firstconv = resnet.conv1      #训练三通道数据
        # Add a new convolutional layer to handle four channels input，处理四通道数据，修改卷积层，并将预训练权重赋值给新的卷积
        self.firstconv= nn.Conv2d(num_channels, 64, kernel_size=7, stride=2, padding=3, bias=False)
        self.firstconv.weight.data[:, :3] = resnet.conv1.weight.data
        self.firstconv.weight.data[:, 3] = resnet.conv1.weight.data.mean(dim=1)
...
# 定义一个输入张量
    input_tensor = torch.randn(2, 4, 512, 512).to(device)
