...
class ResNet(nn.Module):  # renet网络的构成部分
    ...
    def _load_pretrained_model(self):
        pretrain_dict = model_zoo.load_url('https://download.pytorch.org/models/resnet18-5d3b4d8f.pth')
        model_dict = {}
...
def ResNet18(nInputChannels=3, os=16, pretrained=False):
...
class DeepLabv3_plus(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=21, os=16, pretrained=False, freeze_bn=False, _print=False):
        if _print:
        ...
        self.n_channels = nInputChannels
        self.n_classes  = n_classes

        # Atrous Conv  首先获得从resnet18中提取的features map
        self.resnet_features = ResNet18(nInputChannels, os, pretrained=pretrained)
...
if __name__ == "__main__":
    model = DeepLabv3_plus(nInputChannels=3, n_classes=7)
    print(model)
    model.eval()
