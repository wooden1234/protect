...
if __name__ == '__main__':
    ...
    # ----------------------------------------------------#
    model_name = "hornet_unet_ch4321gf_0.6_100_tiny_7×7_self.pth"
    tpye_model = model_name .strip().split('_')[0]+'_unet'
    print(tpye_model)
    ...
    dir = os.path.abspath('.')
    print(dir)
    model_path= "D:\python\\unetNew\dataset-sample-self\ch4321gf\data_1\logs\hornet_unet\checkpoints\hornet_unet_ch4321gf_0.6_100_tiny_7×7_self.pth"
    print("加载模路径：")
    n_classes=2
    ...
    #dir = 'dataset-sample-self/' + model_name.split("_")[2]
    # 训练unet_se和unet_v1和deeplabv3_plus用上面这条
    test_image_dir = dir + '/data_1/test'
    test_filename = dir + "/data_1/test.txt"
    #labsave_path保存预测标签路径,savepath保存结果
    labsave_path = dir + '/data_1/test/label/'+model_name+'/'
    if not os.path.exists(labsave_path ):
        os.makedirs(labsave_path )
    savepath = dir + '/data_1/test/logs/'
